...
wv_og = float(input('Water Vapour, choose from 1.0, 1.6, 3.0, 5.0 mm (default is 1.6): ') or '1.6')
...
Temp_Env = float(input('Instrument Temperature (default is 273K):') or '273')
...
def wfi_cs_limiting_mag(filter_name, filter_wc, filter_dw, Tr_filter, instrument, hrs, snr, rns, pxs):
    ...
    for i in range(len(filter_name)):
        ...
        #Telescope Thermal background component
        ...
        flam1 = 2.0 * snr * p * math.sqrt( skybg_flam1 + telbg_flam1 + moonbg_flam1 + rnbg_flam1 ) / (A * texp * eta * filter_dw[i]*1.0e+4)
        flam2 = 2.0 * snr * p * math.sqrt( skybg_flam2 + telbg_flam2 + moonbg_flam2 + rnbg_flam2 ) / (A * texp * eta * filter_dw[i]*1.0e+4)
        flam3 = 2.0 * snr * p * math.sqrt( skybg_flam3 + telbg_flam3 + moonbg_flam3 + rnbg_flam3 ) / (A * texp * eta * filter_dw[i]*1.0e+4)
        flam4 = 2.0 * snr * p * math.sqrt( skybg_flam4 + telbg_flam4 + moonbg_flam4 + rnbg_flam4 ) / (A * texp * eta * filter_dw[i]*1.0e+4)
        flam5 = 2.0 * snr * p * math.sqrt( skybg_flam5 + telbg_flam5 + moonbg_flam5 + rnbg_flam5 ) / (A * texp * eta * filter_dw[i]*1.0e+4)
        flam6 = 2.0 * snr * p * math.sqrt( skybg_flam6 + telbg_flam6 + moonbg_flam6 + rnbg_flam6 ) / (A * texp * eta * filter_dw[i]*1.0e+4)

        #Sensitivity (limiting magnitude) in AB mags
        m1 = -2.5*math.log10(flam1) - 5.0*math.log10(filter_wc[i]*1.0e+4) - 2.408 #GLAO Limiting mag in 25% (good) seeing condition
        m2 = -2.5*math.log10(flam2) - 5.0*math.log10(filter_wc[i]*1.0e+4) - 2.408 #GLAO Limiting mag in 50% (moderate) seeing condition
        m3 = -2.5*math.log10(flam3) - 5.0*math.log10(filter_wc[i]*1.0e+4) - 2.408 #GLAO Limiting mag in 75% (bad) seeing condition
        m4 = -2.5*math.log10(flam4) - 5.0*math.log10(filter_wc[i]*1.0e+4) - 2.408 #NOAO Limiting mag in 25% (good) seeing condition
        m5 = -2.5*math.log10(flam5) - 5.0*math.log10(filter_wc[i]*1.0e+4) - 2.408 #NOAO Limiting mag in 50% (moderate) seeing condition
        m6 = -2.5*math.log10(flam6) - 5.0*math.log10(filter_wc[i]*1.0e+4) - 2.408 #NOAO Limiting mag in 75% (bad) seeing condition
...
def plot_sens(lm, filters, ao, instrument, snr, hrs):
    ...
    if ao == 'glao':
        ...
        title = r"$\bf{WFI\ (GLAO)\ Airmass="+str(am_og)+"\ Water\ Vapor="+str(wv_og)+"mm}$"
    else:
        y = [n25_arr, n50_arr, n75_arr]
        title = r"$\bf{WFI\ (NOAO)\ Airmass="+str(am_og)+"\ Water\ Vapor="+str(wv_og)+"mm}$"

    plt.errorbar(wc_arr, y[0], xerr=dw_arr, fmt='o', ecolor='green', capsize=4, markersize=10,  color = "green", label='Good seeing (25%)')
    plt.errorbar(wc_arr, y[1], xerr=dw_arr, fmt='D', ecolor='blue', capsize=4, markersize=10,  color = "blue", label='Moderate seeing (50%)')
    plt.errorbar(wc_arr, y[2], xerr=dw_arr, fmt='p', ecolor='red', capsize=4, markersize=10,  color = "red", label='Poor seeing (75%)')
    
    for i, txt in enumerate(name):
        plt.annotate(txt, (wc_arr[i], y[2][i]), xytext=(wc_arr[i]-lgd_dim[0], y[2][i]-lgd_dim[1]))
#        plt.annotate(txt, (wc_arr[i], y[2][i]), xytext=(wc_arr[i]-lgd_dim[0], y[2][i]-lgd_dim[1]), fontsize=5, arrowprops=dict(arrowstyle='->', lw=1))

    plt.xlabel(r"$\bf{Wavelength\ [micron]}$")
    plt.ylabel(r"$\bf{"+str(snr)+"\sigma\ Limiting\ AB\ Mag\ (exp\ time =" + str(hrs) + "\ hrs)}$")
#    plt.xlim(1.1,2.4)
#    plt.ylim(24.1,25.4)
    plt.grid()
